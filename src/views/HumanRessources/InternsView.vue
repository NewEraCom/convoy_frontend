...
watch(rhStore.interns, (newValue) => {
  interns.value = newValue;
}, { deep: true });

const deleteStg = async () => {
  isLoading.value = true;
  console.log(rhStore.ItemId);
  
  await internsService.deleteIntern(rhStore.ItemId).then(() => {
...
const ValidatePotfunc = async () => {
  ...
  await internsService.potentielIntern(rhStore.ItemId, formData).then(() => {
    isLoading.value = false;
...
const RemovePotfunc = async () => {
  isLoading.value = true;
  const formData = new FormData();
  formData.append('potentiel', String(0));
  await internsService.potentielIntern(rhStore.ItemId, formData).then(() => {
    isLoading.value = false;
...
<template>
  <div class="flex-grow-1 container-fluid mt-3">
    ...
    <div v-if="interns.stats" class="row g-3">
      ...
      <div class="col-12">
        <div class="card">
          <div class="card card-border-shadow-primary">
            ...
            <div v-if="interns.data" class="card-body border-top pt-4">
