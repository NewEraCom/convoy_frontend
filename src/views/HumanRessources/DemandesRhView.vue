...
import { demandeService,employeeService } from '@/services/v2';
...
const ReceivedDmnd = async () => {
  ...
  await demandeService.update(rhStore.ItemId,formData).then(() => {
...
const DeleteDmnd = async () => {
  isLoading.value = true;
  await demandeService.delete(rhStore.ItemId).then(() => {
    isLoading.value = false;
