...
import { projectService } from '@/services/v2/production';
...
const projects = ref(computed(() => prodStore.projectsList));
onMounted(async () => {
...
const submit = async () => {
    ...
    await employeeService.AffecterEmpProjet(props.emp_id,formData.value).then(() => {
                        ...
                        <div class="mb-3">
                            ...
                            <div v-if="projects != null">
                                <CustomSelect v-model="formData.project_id" placeholder="Choisir un projet"
                                    label="Projets" :data="projects.filter(item => item.status == 'in_progress').map((item) => ({
                                        key: item.id,
                                        value: item.code
                                    }))
                                " />
