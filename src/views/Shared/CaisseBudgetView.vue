...
onMounted(async () => {
  await eventService.getCaisseSiege('siege');
...
const DeleteCaisse = async () => {
  isLoading.value = true;

 if(sharedStore.selectedItem.validation_DG == 'pending' 
 && sharedStore.selectedItem.validation_DO == 'pending' 
 && sharedStore.selectedItem.validation_DS == 'pending' ){
   await eventService.deleteBudget(sharedStore.selectedItem.id).then(() => {
     isLoading.value = false;
  ...
  toast.error('Un des Directeurs a validé la demande, il ne peut pas être supprimé.');
}
 };
  // console.log($('#validateInput').val());
</script>
<template>
  <div class="flex-grow-1 container-fluid mt-3">
    ...
    <div v-if="budget.stats" class="row g-3">
          <BudgetCard  :stats="budget.stats"/>
      ...
      <div class="col-12">
        <div class="card">
          <div class="card card-border-shadow-primary">
            ...
            <div v-if="budget.data" class="card-body border-top pt-4 overflow-auto">
              <BudgetTable :budget="budget.data" />
            </div>
    ...
    <Validate id="reject-modal" :isLoading="isLoading" :method="RejectCaisse" 
      ...
      <Validate id="delete-modal" :isLoading="isLoading" :method="DeleteCaisse" 
      title="Supprimmer Caisse" message="Êtes-vous sûr de supprimmer cet caisse" severity="danger" />
  </div>
</template>
