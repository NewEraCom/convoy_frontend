...
const headers = [
    { text: 'Créer par', type: 'recrute',isComplex: true },
    { text: 'Project', isComplex: true, type: 'project' },
    { text: 'Mois', value: 'month', type: 'text' },
...
const filter = () => {
    filteredData.value = props.budget.filter((item: any) => {
        const combinedFields = `${item.created_by.employee.first_name} ${item.created_by.employee.last_name} ${item.designation} ${item.mois} ${item.status}`.toLowerCase();
