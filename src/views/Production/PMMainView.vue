...
import { projectService,purchaseService } from '@/services/v2';
...
const demande_achat = ref(computed(() => productionStore.purchaseOrders.data));

const data = ref(computed(() => productionStore.projectsStats));

onMounted(async () => {
  await projectService.getDataManager();
  await purchaseService.getPurchaseOrdersByProjectManager('achat');
});

onUnmounted(() => {
  productionStore.clearPurchaseOrders();
  productionStore.clearStats();
});
              ...
              <tbody v-if="demande_achat.length != 0">
                <tr v-for="item in demande_achat" :key="item.id">
                  ...
                  <td class="fw-bold text-center">
                    <small>{{ item.pre_project.project.code }}</small>
                  </td>
