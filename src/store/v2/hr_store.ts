  ...
  actions: {
    ...
    setEmployees(data) {
      ...
      this.stats = {
        ...
        cdi: data.employee.filter((e: any) => e.status == 'active' && e.type_contrat == 'CDI')
          .length,
        cdd: data.employee.filter((e: any) => e.status == 'active' && e.type_contrat == 'CDD')
          .length,
        chantier: data.employee.filter(
          (e: any) => e.status == 'active' && e.type_contrat === 'Chantier'
        ).length,
        commercial: data.employee.filter(
          (e: any) => e.status == 'active' && e.type_contrat === 'COMMERCIALE'
        ).length,
      ...
      this.employeeExpire = data.employee.filter((e: any) => {
        ...
        return (
          e.status === '1' && contractEndYear === currentYear && contractEndMonth === currentMonth
        );
      });
    },
    clearEmployees() {
    ...
    pushRecrutement(data: any) {
      ...
      this.recrutement.stats = {
        ...
        accepted: this.recrutement.data.filter((e: any) => e.status === 'approved').length
      };
    },
    deleteRecruitment(id: number) {
      const itemIdToDelete = id;
      const indexToDelete = this.recrutement.data.findIndex(
    ...
    pushDemande(data: any) {
      this.demandeRh.data?.unshift(data);
      this.demandeRh.stats = {
        total: this.demandeRh.length,
        pending: this.demandeRh.filter((e: any) => e.status === 'pending').length,
        done: this.demandeRh.filter((e: any) => e.status === 'done').length,
        delivered: this.demandeRh.filter((e: any) => e.status === 'delivered').length
      };
    },
    setSalaryAdvances(data: any) {
      ...
      this.salaryAdvances.stats = {
        total: this.salaryAdvances.data.reduce((accumulator: number, current: any) => {
          let total = 0;
          if (current.approval_rh === 'approved') {
    ...
    pushSalaryAdvance(data) {
      // Create a new array with the new data
      const newData = [...this.salaryAdvances.data, data];

      // Replace the old array with the new one
      this.salaryAdvances.data = newData;
