...
export const useLogisticsStore = defineStore('LogisticsStore', {
    ...
    actions: {
        ...
        updateStatistics() {
            const data = this.louer.data;
            // Recalculate statistics
            this.louer.stats = {
                appartement: data.filter(item => item.type === 'Appartement' && item.a_payer == 1).length ?? 0,
                depot: data.filter(item => item.type === 'Dépôt' && item.a_payer == 1).length ?? 0,
                magasin: data.filter(item => item.type === 'Magasin'&& item.a_payer == 1).length ?? 0,
                rejected: data.filter(item => item.type === 'Autre'&& item.a_payer == 1).length ?? 0,
                villa: data.filter(item => item.type === 'Villa'&& item.a_payer == 1).length ?? 0,
                total_a_payer: data.reduce((acc: any, item: any) => {
                            if (item.a_payer == 1) {
        ...
        updatedLouer(data){
            const index =  this.louer.data.findIndex(t => t.id === data.id);
            if (index !== -1) {
                 this.louer.data[index] = data;
                
                 
            } else {
                 this.louer.data.push(data);
        ...
        setLouer(data: any) {
            this.louer.data = data;
            // this.louer.stats = {
            //     appartement: data.louer.reduce((acc: any, item: any) => {
        ...
        pushLouer(data: any) {
            this.selectedItem.paiements?.push(data) 
          
        },
        validateLouer(data){
            data.a_payer = data.a_payer ? 1 : 0;
            const index =  this.louer.data.findIndex(t => t.id === data.id);
            if (index !== -1) {
                 this.louer.data[index] = data;
        ...
        deleteRent(id: number){
            ...
            const indexToDelete =  this.selectedItem.paiements.findIndex((item: any) => item.id == itemIdToDelete);
            if (indexToDelete !== -1) {
                this.selectedItem.paiements.splice(indexToDelete, 1);
            } else {
        ...
        deletePaiement(id: number){
            const itemIdToDelete = id;
            const indexToDelete = this.louer.data.findIndex((item: any) => item.id == itemIdToDelete);
            if (indexToDelete !== -1) {
                this.louer.data.splice(indexToDelete, 1);
            } else {
                console.log('Item not found in array.');
            }
        },
        ...
        setSelectedItem(data: any) {
            data.paiements.payer = data.paiements.payer ? 1 : 0;
            this.selectedItem = data;
        },
        clearSelectedItem() {
            this.selectedItem = null;
